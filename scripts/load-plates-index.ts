...
async function main() {
  ...
  // Init schema
  const flag = isRemote ? "--remote" : "--local";
  execSync(`npx wrangler d1 execute betaplanes-db ${flag} --command="CREATE TABLE IF NOT EXISTS faa_plates (id INTEGER PRIMARY KEY AUTOINCREMENT, airport_ident TEXT NOT NULL, icao_ident TEXT, state TEXT, city TEXT, chart_code TEXT NOT NULL, chart_name TEXT NOT NULL, pdf_name TEXT NOT NULL); CREATE INDEX IF NOT EXISTS idx_faa_plates_airport ON faa_plates(airport_ident); CREATE INDEX IF NOT EXISTS idx_faa_plates_icao ON faa_plates(icao_ident);"`, { stdio: "pipe" });
