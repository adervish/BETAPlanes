...
async function main() {
  ...
  execSync(`npx wrangler d1 execute betaplanes-db ${flag} --command="CREATE TABLE IF NOT EXISTS faa_frequencies (id INTEGER PRIMARY KEY AUTOINCREMENT, airport_ident TEXT NOT NULL, service_type TEXT, freq_tx REAL, freq_rx REAL, remarks TEXT); CREATE INDEX IF NOT EXISTS idx_faa_freq_airport ON faa_frequencies(airport_ident);"`, { stdio: "pipe" });

  // Delete only UNICOM/CTAF/TOWER entries (keep ATIS/AWOS from previous load)
  if (isRemote) {
    execSync(`npx wrangler d1 execute betaplanes-db --remote --command="DELETE FROM faa_frequencies WHERE service_type IN ('UNICOM','CTAF','TOWER')"`, { stdio: "pipe" });
  }
  ...
  if (isRemote) {
    for (let i = 0; i < lines.length; i++) {
      ...
      try {
        execSync(`npx wrangler d1 execute betaplanes-db --remote --file=${tmpFile}`, {
