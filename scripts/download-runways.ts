...
async function main() {
  ...
  execSync(`npx wrangler d1 execute betaplanes-db ${flag} --command="CREATE TABLE IF NOT EXISTS faa_runways (id INTEGER PRIMARY KEY AUTOINCREMENT, airport_ident TEXT NOT NULL, designator TEXT, length_ft INTEGER, width_ft INTEGER, surface TEXT, lighting TEXT); CREATE INDEX IF NOT EXISTS idx_faa_runways_airport ON faa_runways(airport_ident);"`, { stdio: "pipe" });
  ...
  // Delete and insert
  if (isRemote) {
    execSync(`npx wrangler d1 execute betaplanes-db --remote --command="DELETE FROM faa_runways"`, { stdio: "pipe" });
  }
  ...
  if (isRemote) {
    ...
    for (let i = 0; i < lines.length; i += CHUNK) {
      ...
      try {
        execSync(`npx wrangler d1 execute betaplanes-db --remote --file=${chunkFile}`, {
    ...
    try {
      execSync(`npx wrangler d1 execute betaplanes-db --local --file=${tmpFile}`, {
        stdio: "pipe", timeout: 120000, maxBuffer: 50 * 1024 * 1024,
      });
    } catch (e: any) {
      const stdout = e.stdout?.toString() || "";
